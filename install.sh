...
# 0. Pastikan dijalankan sebagai root
if [ "$EUID" -ne 0 ]; then
    echo "❌ Script ini harus dijalankan sebagai root (gunakan sudo atau login root)."
    exit 1
fi
...
# Generate API key acak untuk mengamankan endpoint /send-notification
# (dibuat sekali; jika sudah ada dari instalasi sebelumnya, pakai yang lama)
if [ -f /root/wa-notif-bot/apikey.txt ]; then
    API_KEY=$(cat /root/wa-notif-bot/apikey.txt)
else
    API_KEY=$(head -c 32 /dev/urandom | sha256sum | cut -d' ' -f1)
    echo "$API_KEY" > /root/wa-notif-bot/apikey.txt
    chmod 600 /root/wa-notif-bot/apikey.txt
fi
...
// Muat API key untuk otentikasi endpoint
let API_KEY = '';
try {
    API_KEY = fs.readFileSync('./apikey.txt', 'utf8').trim();
} catch (e) {
    console.error('⚠️  apikey.txt tidak ditemukan. Endpoint /send-notification tidak akan bisa diakses.');
}

let sock;
let reconnectAttempts = 0;
const MAX_RECONNECT_DELAY_MS = 60000; // cap 60 detik
...
app.post('/send-notification', async (req, res) => {
    ...
    if (!API_KEY || providedKey !== API_KEY) {
        return res.status(401).json({ status: false, message: 'Unauthorized. Header x-api-key tidak valid.' });
    }
...
while true; do
    ...
    echo "5. Tampilkan API Key"
    echo "6. Hapus Semua Script (Uninstall Bersih)"
    echo "7. Keluar"
    echo "========================================="
    ...
    case $pilihan in
        ...
        5)
            echo ""
            echo "=== API KEY ENDPOINT /send-notification ==="
            if [ -f /root/wa-notif-bot/apikey.txt ]; then
                echo "x-api-key: $(cat /root/wa-notif-bot/apikey.txt)"
            else
...
echo ""
echo "🔑 API Key untuk endpoint /send-notification:"
echo "   $API_KEY"
echo "   (tersimpan di /root/wa-notif-bot/apikey.txt)"
echo "   Sertakan sebagai header: x-api-key: $API_KEY"
echo "=================================================="
